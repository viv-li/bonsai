<template>
  <FormPage>
    <h1 class="title">How much would you like to transfer?</h1>
    <h2 class="subtitle">
    ...
    <div class="field" :class="[transferAmount > 0? '': 'is-invisible']">
      <p class="subtitle">
        ...
        <b>${{ parseFloat(transferAmount).toFixed(2) }}</b> to your Bonsai cash balance
      </p>
      <a
...
export default {
  ...
  methods: {
    transferCash() {
      ...
      this.$store
        .dispatch("transferCash", parseFloat(this.transferAmount))
        .then(
          () => {
